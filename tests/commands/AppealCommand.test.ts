...
describe("AppealCommand behavior", () => {
  it("handles submit interaction lifecycle from select to modal creation", async () => {
    ...
    (context.databases.userDb.GetWarnings as ReturnType<typeof vi.fn>).mockReturnValue([
      {
        id: 21,
        user_id: "user-1",
        guild_id: "guild-1",
    ...
    (context.databases.userDb.GetWarningById as ReturnType<typeof vi.fn>).mockReturnValue(
      {
        id: 21,
        user_id: "user-1",
        guild_id: "guild-1",
    ...
    (context.databases.moderationDb.AddAppeal as ReturnType<typeof vi.fn>).mockReturnValue(
      {
        id: 55,
        guild_id: "guild-1",
        user_id: "user-1",
    ...
    const selectRegistration = (
      context.responders.selectMenuRouter.RegisterSelectMenu as ReturnType<typeof vi.fn>
    ).mock.calls[0][0];
    ...
    expect(context.responders.modalRouter.RegisterModal).toHaveBeenCalledTimes(1);
    ...
    const editPayload = (modalInteraction.editReply as ReturnType<typeof vi.fn>).mock
      .calls[0][0] as { embeds: Array<{ title?: string }> };
