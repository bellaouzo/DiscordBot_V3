...
export class CommandConfigBuilder {
  ...
  anyPermission(...perms: (keyof typeof PermissionFlagsBits)[]): this {
    this.config = { 
      ...this.config, 
      permissions: { ...this.config.permissions, required: perms, requireAny: true }
    }
    return this
  }

  cooldownSeconds(seconds: number): this {
    this.config = { ...this.config, cooldown: { ...this.config.cooldown, seconds } }
    return this
  }

  cooldownMinutes(minutes: number): this {
    this.config = { ...this.config, cooldown: { ...this.config.cooldown, minutes } }
    return this
  }

  cooldownMs(milliseconds: number): this {
    this.config = { ...this.config, cooldown: { ...this.config.cooldown, milliseconds } }
    return this
  }
...
export const Config = {
  ...
  requirePermissions: (...perms: (keyof typeof PermissionFlagsBits)[]) =>
    CommandConfigBuilder.create().permissions(...perms).build(),
  
  requireAnyPermission: (...perms: (keyof typeof PermissionFlagsBits)[]) =>
    CommandConfigBuilder.create().anyPermission(...perms).build(),
  
  requireRole: (roleId: string) =>
    CommandConfigBuilder.create().role(roleId).build(),
  
  requireOwner: () =>
    CommandConfigBuilder.create().owner().build(),
