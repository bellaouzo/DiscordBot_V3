...
export function createApiRouter(
    ...
    for (const g of mutualGuildsRaw) {
      ...
      try {
        if (g.owner) {
          ...
          if ((perms & administrator) === administrator || (perms & manageGuild) === manageGuild) {
            isAdmin = true;
          }
        }
  ...
  router.get("/tickets/:guildId/:ticketId", async (req, res) => {
    ...
    const enrichedTicket = {
      ...
      closed_by_username: ticket.claimed_by ? (usernameMap.get(ticket.claimed_by) || "Unknown User") : null
    };
  ...
  router.get("/infractions/:guildId", (req, res) => {
    ...
    const kicks = databases.moderationDb.ListModerationEvents({ guild_id: guildId, user_id: user.id, action: "kick" });
    const bans = databases.moderationDb.ListModerationEvents({ guild_id: guildId, user_id: user.id, action: "ban" });
    const mutes = databases.moderationDb.ListUserTempActions({ guild_id: guildId, user_id: user.id, action: "mute" });
  ...
  router.get("/admin/tickets/:guildId", verifyAdmin, (req, res) => {
    ...
    res.json([...openTickets, ...closedTickets].sort((a, b) => a.created_at - b.created_at));
  });
  ...
  router.get("/admin/infractions/:guildId", verifyAdmin, (req, res) => {
    ...
    try {
      const events = db.prepare("SELECT * FROM moderation_events WHERE guild_id = ? ORDER BY created_at DESC LIMIT 100").all(guildId);
      const tempActions = db.prepare("SELECT * FROM temp_actions WHERE guild_id = ? ORDER BY created_at DESC LIMIT 100").all(guildId);
  ...
  router.post("/admin/tickets/:guildId/close/:ticketId", verifyAdmin, (req, res) => {
    const ticketId = parseInt(req.params.ticketId as string);
    const reason = req.body?.reason || null;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const success = databases.ticketDb.CloseTicket(ticketId, (req.user as any).id, reason);
    res.json({ success });
  });
  ...
  router.get("/admin/user/:guildId/:userId", verifyAdmin, async (req, res) => {
    ...
    const kicks = databases.moderationDb.ListModerationEvents({ guild_id: guildId, user_id: userId, action: "kick" });
    const bans = databases.moderationDb.ListModerationEvents({ guild_id: guildId, user_id: userId, action: "ban" });
    const mutes = databases.moderationDb.ListUserTempActions({ guild_id: guildId, user_id: userId, action: "mute" });
  ...
  router.post("/admin/settings/:guildId", verifyAdmin, (req, res) => {
    ...
    const settingsToSave = {
      ...
      starboard_threshold: typeof body.starboard_threshold === "number" 
        ? body.starboard_threshold 
        : body.starboard_threshold ? parseInt(String(body.starboard_threshold)) : null,
      ...
      verification_min_account_age_days: typeof body.verification_min_account_age_days === "number"
        ? body.verification_min_account_age_days
        : body.verification_min_account_age_days ? parseInt(String(body.verification_min_account_age_days)) : 0,
